...
"""
    ...
    if "export const metadata" in content:
        # robust replacement
        # find the start of export const metadata
        start_idx = content.find("export const metadata")
        # if createSeoMetadata is already used, continue to next
        if "createSeoMetadata(" in content[start_idx:start_idx+100]:
            print(f"Skipping {file_path}")
            continue
            
        # find the next export or end of file
        next_export = content.find("export ", start_idx + 20)
        if next_export == -1:
            content = content[:start_idx] + metadata_str
        else:
            content = content[:start_idx] + metadata_str + "\n" + content[next_export:]
    else:
