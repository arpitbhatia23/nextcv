...
// ✅ Extract static data outside the component to prevent re-creation
const RESUME_MOCKUPS = [
  {
    name: "Modern",
...
// ✅ Memoized sub-component for each resume mockup
const ResumeMockup = React.memo(function ResumeMockup({ mockup, isActive }) {
  return (
    <section
    ...
    >
      ...
      >
        <Image
          ...
          width={mockup.width}
          alt={`${mockup.name} resume template`}
...
function AnimatedHeroSection() {
  ...
  // ✅ useCallback to avoid re-creating function
  ...
  // ✅ Reduced interval frequency & clean effect
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentMockup((prev) => (prev + 1) % RESUME_MOCKUPS.length);
    }, 5000);
    return () => clearInterval(timer);
  }, []);

  // ✅ useMemo for map-heavy lists
  const mockupButtons = useMemo(
    () =>
      RESUME_MOCKUPS.map((_, index) => (
        <button
          key={index}
          ...
          aria-label={`Switch to ${RESUME_MOCKUPS[index].name}`}
        />
      )),
    [currentMockup]
  );
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-purple-900 text-white py-20 px-4 overflow-hidden ">
      {/* Background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl animate-pulse delay-1000" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-80 h-80 bg-pink-500/5 rounded-full blur-3xl animate-pulse delay-2000" />
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center min-h-[80vh]">
          ...
          <div className="space-y-8 lg:pr-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                className="group bg-gradient-to-r from-yellow-400 cursor-pointer to-orange-500 text-gray-900 font-bold py-4 px-8 rounded-2xl text-lg shadow-2xl transform hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2"
              ...
              <button
                className="group bg-white/10 backdrop-blur-sm border-2 border-white/30 text-white font-bold py-4 px-8 rounded-2xl text-lg hover:bg-white hover:text-gray-900 transition-all duration-300 "
          ...
          <div className="hidden sm:flex justify-center relative">
            <div className="relative">
              <div className="absolute -inset-8 bg-gradient-to-r from-blue-500/20 to-purple-500/20 rounded-3xl blur-2xl animate-pulse pointer-events-none" />
              <div className="relative flex items-center justify-center min-h-[500px]">
                {RESUME_MOCKUPS.map((mockup, index) => (
                  <div
                    key={mockup.name}
                    className={`absolute transition-all duration-1000 ${
                      index === currentMockup
                        ? "z-20"
                        : index === (currentMockup + 1) % RESUME_MOCKUPS.length
                        ? "z-10"
                        : "z-0"
                    }`}
                    style={{
                      transform:
                        index === currentMockup
                          ...
                          : index ===
                            (currentMockup + 1) % RESUME_MOCKUPS.length
