    ...
    <section name="How It Works" className="py-24 bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="relative">
          ...
          <div className="hidden lg:block absolute top-12 left-0 w-full h-0.5 bg-linear-to-r from-indigo-200 via-purple-200 to-emerald-200 -z-10 transform -translate-y-1/2" />

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            {steps.map((step, index) => (
              <div key={index} className="relative group">
                <div
                  className={`w-14 h-14 rounded-2xl ${step.color} border flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300 relative z-10 bg-white`}
                >
                  {step.icon}
