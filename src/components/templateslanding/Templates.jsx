...
const Templates = () => {
  const cards = [
    { img: '', title: 'Executive Pro', description: 'Perfect for senior professionals' },
    { img: '', title: 'Creative Edge', description: 'Ideal for creative professionals' },
    { img: '', title: 'Tech Modern', description: 'Great for tech professionals' },
    { img: '', title: 'Clean Minimal', description: 'Simple and professional' },
    { img: '', title: 'Student Fresh', description: 'Perfect for new graduates' },
  ];
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {cards.map(({ img, title, description }, index) => (
              <Card key={index} className="p-4">
                <CardHeader>{img || <div className="h-32 bg-gray-200 rounded" />}</CardHeader>
      ...
      <section className="px-4 py-12 ">
        <Card className="max-w-screen-lg mx-auto border-none shadow-none space-y-6">
          <h1 className="text-2xl font-bold text-center">All Templates Include</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 bg-blue-100 dark:bg-black p-6 rounded-lg">
            {templates_includ.map(({ icon, title, description }, index) => (
              <Card key={index} className="flex flex-col items-center text-center bg-blue-100 dark:bg-black border-none shadow-none">
          ...
          <div className="flex flex-col items-center space-y-4">
            <Button className="bg-blue-600 hover:bg-blue-700 text-white text-lg px-6 py-3 w-full sm:w-auto">
              Create your Resume
            </Button>
