            ...
            <SidebarMenuItem key={item.title}>
              <Link href={item?.url}>
                <SidebarMenuButton
                  tooltip={item.title}
                  className={
                    "flex items-center justify-start gap-3 py-6 px-4 hover:bg-slate-50 rounded-xl transition-all group"
                  }
                >
                  <span className="text-xl text-center group-hover:scale-110 transition-transform">
                    {item.icon && <item.icon size={22} />}
                  </span>{" "}
                  <span className="text-[15px] font-medium text-slate-700 group-hover:text-indigo-600 transition-colors">
                    {item.title}
                  </span>
                  {item.badge && (
