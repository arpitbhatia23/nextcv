             ...
             <div className="px-6 md:px-12 py-10">
                ...
                <div className="flex items-center gap-4 py-6 border-y border-slate-100 mb-10">
                    <div className="relative w-12 h-12 rounded-full overflow-hidden bg-slate-100 ring-2 ring-white shadow-sm">
                        {blog.author?.image ? (
                            <Image src={urlFor(blog.author.image).width(100).url()} alt={blog.author.name} fill className="object-cover" />
                        ) : (
                            <User className="w-6 h-6 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-slate-400" />
                        )}
                    </div>
                    <div>
                        <p className="text-base font-semibold text-slate-900">{blog.author?.name || "NextCV Team"}</p>
                        <p className="text-sm text-slate-500">Author</p>
                    </div>
                    
                    {/* Share Buttons (Inline for mobile/Desktop) */}
                    <div className="ml-auto flex gap-2">
                        <button onClick={() => handleShare("twitter")} className="p-2 text-slate-400 hover:text-sky-500 hover:bg-sky-50 rounded-full transition-colors"><Twitter className="w-5 h-5" /></button>
                        <button onClick={() => handleShare("linkedin")} className="p-2 text-slate-400 hover:text-blue-700 hover:bg-blue-50 rounded-full transition-colors"><Linkedin className="w-5 h-5" /></button>
                        <button onClick={() => handleShare("copy")} className="p-2 text-slate-400 hover:text-green-600 hover:bg-green-50 rounded-full transition-colors relative">
                ...
                <article className="prose prose-lg prose-slate max-w-none 
                    ...
                    <Suspense fallback={<div className="space-y-4 animate-pulse"><div className="h-4 bg-slate-200 rounded w-3/4"></div><div className="h-4 bg-slate-200 rounded"></div><div className="h-4 bg-slate-200 rounded w-5/6"></div></div>}>
