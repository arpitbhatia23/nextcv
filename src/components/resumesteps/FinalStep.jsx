...
import { BadgePercent, IndianRupee, Save, Download, LayoutTemplate, CheckCircle2, AlertCircle, FileText } from "lucide-react";
    ...
    <div className="py-4 h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="w-full flex flex-col gap-4 md:hidden pb-20">
              ...
              <div className="flex overflow-x-auto gap-3 pb-2 scrollbar-hide snap-x">
                {templates.map((template) => (
                  <div 
                    ...
                    className={`flex-none w-[160px] flex flex-col items-center gap-2 p-3 rounded-lg border transition-all cursor-pointer snap-start ${
                        selectedTemplate === template.key 
                        ? "bg-indigo-50 border-indigo-500 shadow-sm" 
                        : "bg-white border-slate-200 hover:border-slate-300"
                    }`}
                  >
                     <div className={`p-3 rounded-full ${selectedTemplate === template.key ? "bg-indigo-100 text-indigo-600" : "bg-slate-100 text-slate-500"}`}>
                        <LayoutTemplate className="w-6 h-6" />
                     </div>
                     <div className="w-full text-center">
                        <p className={`text-xs font-medium whitespace-normal text-center leading-tight ${selectedTemplate === template.key ? "text-indigo-900" : "text-slate-700"}`}>
                            {template.label}
                        </p>
                     </div>
                     {selectedTemplate === template.key && <CheckCircle2 className="w-4 h-4 text-indigo-600 mt-1" />}
                  </div>
                ))}
              </div>
           </div>
        ...
        <Card className="border border-slate-200 shadow-sm overflow-hidden bg-white">
          ...
          <CardContent className="p-0 bg-slate-100/50 min-h-[300px] flex items-center justify-center">
              {pdfUrl ? (
                <div className="p-4 w-full flex justify-center">
                    <Document
                    file={pdfUrl}
                    loading={<div className="text-slate-400 text-sm animate-pulse">Loading Preview...</div>}
                    onLoadSuccess={({ numPages }) => setNumPages(numPages)}
                    className="shadow-md"
                    >
                    {Array.from({ length: numPages || 0 }).map((_, idx) => (
                        <Page key={idx} pageNumber={idx + 1} width={280} className="mb-2" renderAnnotationLayer={false} renderTextLayer={false} />
                    ))}
                    </Document>
                </div>
              ) : (
          ...
          <CardContent className="p-4 space-y-5">
             ...
             <div className="space-y-2">
                 <div className="flex gap-2">
                    ...
                    {applied && (
                        <Button variant="ghost" size="icon" onClick={removeCoupon} className="h-10 w-10 shrink-0 text-red-500 hover:bg-red-50">
                 ...
                 {applied && discount && (
                    <p className="text-xs text-green-600 font-medium flex items-center gap-1 pl-1">
                        ...
                        Saved {discount.type === "percentage" ? `${discount.value}%` : `₹${discount.value}`}
                    </p>
                 )}
             </div>
      ...
      <div className="hidden md:flex flex-1 overflow-hidden w-full gap-6 h-full min-h-0">
          ...
          {!isdraft && (
            <div className="w-80 flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden h-full flex-shrink-0">
                     ...
                     {templates.map((template) => (
                       ...
                       >
                          ...
                          <div className="aspect-[3/4] w-full bg-slate-200 relative overflow-hidden">
                             ...
                             <div className={`absolute inset-0 bg-indigo-900/10 transition-opacity ${selectedTemplate === template.key ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
                          </div>

                          {/* Label */}
                          <div className={`p-3 text-center border-t ${selectedTemplate === template.key ? 'bg-indigo-50 border-indigo-100' : 'bg-white border-slate-100'}`}>
                              <span className={`font-medium text-sm ${selectedTemplate === template.key ? 'text-indigo-700' : 'text-slate-700'}`}>
                                {template.label}
                              </span>
                          </div>
          ...
          <div className="flex-1 flex flex-col bg-slate-50 border border-slate-200 rounded-xl shadow-sm overflow-hidden h-full">
             <div className="p-3 border-b border-slate-200 bg-white flex justify-between items-center px-6">
                <span className="text-sm font-medium text-slate-500 flex items-center gap-2">
                  Live Preview <span className="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 text-xs border border-indigo-100">A4</span>
                </span>
             </div>
                     ...
                     <Document
                       ...
                       loading={
                         <div className="flex flex-col items-center gap-3 p-12 bg-white rounded-xl shadow-sm border border-slate-100">
                            ...
                            <p className="text-sm font-medium text-slate-500 animate-pulse">Rendering Resume...</p>
                         </div>
                       }
                       error={<div className="text-red-500 bg-white p-4 rounded-lg shadow-sm">Failed to load preview</div>}
                     >
                       {Array.from({ length: numPages || 0 }).map((_, idx) => (
                      ...
                      <div className="h-[800px] w-[600px] bg-white animate-pulse rounded-sm shadow-sm border border-slate-200" />
                   )}
                 </div>
             </div>
          </div>
             ...
             <div className="p-5 space-y-6">
                 ...
                 <div className="bg-slate-50 rounded-lg p-4 border border-slate-100 space-y-3">
                    ...
                    {applied && discount && (
                      <div className="flex justify-between items-center text-sm text-green-600">
                         ...
                         <span className="font-semibold">- {discount.type === "percentage" ? `${discount.value}%` : `₹${discount.value}`}</span>
                      </div>
                    )}
                 ...
                 <div className="space-y-2">
                     ...
                     <div className="flex gap-2 relative">
                        ...
                        />
                        {!applied ? (
                           <Button onClick={() => debounceCoupon(couponCode)} disabled={!couponCode.trim() || isSubmit} size="sm" className="bg-slate-800 text-white hover:bg-slate-700 px-4">
                              Apply
                           </Button>
                        ) : (
                           <Button variant="ghost" onClick={removeCoupon} size="icon" className="text-red-500 hover:text-red-600 hover:bg-red-50 absolute right-1 top-1 h-8 w-8">
