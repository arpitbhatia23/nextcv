    ...
    <div className="py-2">
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Credentials & Awards</h2>
          ...
          <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-white rounded-3xl overflow-hidden">
             <div className="bg-indigo-600/5 px-6 py-4 border-b border-indigo-100/50 flex items-center justify-between">
                ...
                {isEditing && (
                  <Button variant="ghost" size="sm" onClick={() => { form.reset(); setIsEditing(false); }} className="h-8 text-xs font-bold text-slate-400">Cancel</Button>
                )}
             </div>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-slate-900 font-bold">Certification Name</FormLabel>
                          <FormControl>
                            <Input placeholder="e.g. AWS Certified Solutions Architect" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Amazon Web Services" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="https://..." {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
        ...
        <div className="lg:col-span-7 space-y-8">
           <div className="flex items-center justify-between">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.35em]">Credential List</h3>
              ...
              <AnimatePresence initial={false}>
                {certList.length === 0 ? (
                  <motion.div 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="col-span-full bg-white border-2 border-dashed border-slate-100 rounded-[2rem] p-16 text-center"
                  >
                    ...
                    <p className="text-slate-300 font-medium text-sm mt-1">Stand out by listing your professional accolades.</p>
                  </motion.div>
                ) : (
                  certList.map((cert) => (
                    <motion.div
                    ...
                    >
                      <Card className="group relative border-none bg-white hover:bg-indigo-50/30 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-xl rounded-[2rem] transition-all overflow-hidden border-2 border-transparent hover:border-indigo-100">
                         <CardContent className="p-6">
                            <div className="flex flex-col h-full justify-between gap-4">
                               <div className="space-y-3">
                                  ...
                                  <h4 className="font-black text-slate-900 leading-snug">{cert.name}</h4>
                                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{cert.issuer}</p>
                               </div>

                               <div className="flex items-center justify-between pt-4 border-t border-slate-50 mt-2">
                                  <span className="text-[10px] font-black text-slate-300">{cert.date}</span>
                                  <div className="flex gap-2">
                                     <button onClick={() => handleEdit(cert)} className="p-2 text-slate-300 hover:text-indigo-600 transition-colors">
                                       <Edit2 className="w-3.5 h-3.5" />
                                     </button>
                                     <button onClick={() => handleDelete(cert.id)} className="p-2 text-slate-300 hover:text-red-500 transition-colors">
