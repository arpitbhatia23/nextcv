    ...
    <div className="py-2">
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Professional History</h2>
            <p className="text-slate-500 mt-2 text-lg">Highlight your career progression and impact.</p>
          </div>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Google, TechStart" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Senior Frontend Engineer" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <div className="flex justify-between items-center mb-1">
                            <FormLabel className="text-slate-900 font-bold">What did you achieve?</FormLabel>
                            <button
                              type="button"
        ...
        <div className="lg:col-span-6 space-y-8">
           <div className="flex items-center justify-between">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.35em]">Career Path</h3>
              ...
              <AnimatePresence initial={false}>
                {experienceList.length === 0 ? (
                  ...
                  >
                    ...
                    <p className="text-slate-400 font-black text-xl leading-snug">No history logged yet.<br/><span className="text-xs font-medium text-slate-300">Your professional growth will appear here.</span></p>
                  </motion.div>
                ) : (
                  experienceList.map((exp, idx) => (
                    <motion.div
                      ...
                      className="relative"
                    >
                      <div className="absolute -left-[27px] top-8 w-4 h-4 rounded-full border-4 border-white shadow-lg bg-indigo-600 z-10" />
                         ...
                         <CardContent className="p-8">
                            <div className="flex items-start justify-between gap-6">
                               <div className="space-y-3">
                                  ...
                                  <h4 className="font-black text-slate-900 text-xl leading-tight">{exp.position}</h4>
                                  <div className="flex items-center gap-3 text-xs font-black text-slate-400">
                                     <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {exp.startDate} - {exp.endDate || "Present"}</span>
                                  </div>
                               </div>

                               <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={() => handleEdit(exp)} className="p-3 bg-white text-slate-400 hover:text-indigo-600 rounded-2xl shadow-sm transition-all">
                                    <Edit2 className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => handleDelete(exp.id)} className="p-3 bg-white text-slate-400 hover:text-red-500 rounded-2xl shadow-sm transition-all">
