    ...
    <div className="py-2">
      <div className="flex flex-col lg:grid lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Innovation Portfolio</h2>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Decentralized Finance Dashboard" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormControl>
                              <Input placeholder="e.g. Lead Architect" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="e.g. Next.js, Web3.js, Tailwind" {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="https://github.com/..." {...field} className="h-12 bg-slate-50 border-transparent focus:bg-white focus:border-indigo-500 rounded-xl transition-all" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <div className="flex justify-between items-center mb-1">
                            <FormLabel className="text-slate-900 font-bold">Impact & Implementation</FormLabel>
                            <button
                              type="button"
        ...
        <div className="lg:col-span-6 space-y-8">
           <div className="flex items-center justify-between">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.35em]">Showcase Index</h3>
              ...
              <AnimatePresence initial={false}>
                {projectList.length === 0 ? (
                  <motion.div 
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="bg-white border-2 border-dashed border-slate-100 rounded-[2rem] p-16 text-center"
                  >
                    ...
                    <p className="text-slate-300 font-medium text-sm mt-1">Add projects to demonstrate domain mastery.</p>
                  </motion.div>
                ) : (
                  projectList.map((project, idx) => (
                    ...
                    >
                      <Card className="group border-none bg-white hover:bg-slate-50/50 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-xl rounded-[2rem] transition-all overflow-hidden">
                         <CardContent className="p-8">
                            <div className="flex items-start justify-between gap-6">
                               <div className="space-y-4 flex-1">
                                  <div className="flex items-center gap-3">
                                     <h4 className="font-black text-slate-900 text-xl leading-tight">{project.title}</h4>
                                     {project.link && (
                                       <a href={project.link} target="_blank" rel="noopener noreferrer" className="p-1.5 bg-slate-100 text-slate-400 hover:text-indigo-600 rounded-lg transition-colors">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                     {project.technologiesOrTopics?.split(",").map((tech, i) => (
                                       <span key={i} className="text-[10px] font-black uppercase tracking-widest bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-lg">
                                         {tech.trim()}
                                       </span>
                                     ))}
                                  </div>
                                  ...
                                  <div className="flex items-center gap-4 text-xs font-black text-slate-400">
                                     <span className="flex items-center gap-1.5 bg-slate-50 px-2 py-0.5 rounded-md italic">"{project.roleOrType}"</span>
                                  </div>
                               </div>

                               <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                  <button onClick={() => handleEdit(project)} className="p-3 bg-white text-slate-400 hover:text-indigo-600 rounded-2xl shadow-sm border border-slate-50 transition-all">
                                    <Edit2 className="w-4 h-4" />
                                  </button>
                                  <button onClick={() => handleDelete(project.id)} className="p-3 bg-white text-slate-400 hover:text-red-500 rounded-2xl shadow-sm border border-slate-50 transition-all">
           ...
           <div className="pt-8 flex justify-between gap-4">
             ...
             <Button onClick={next} className="h-14 px-12 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black shadow-xl shadow-indigo-100 transition-all">
