...
const PageContent = () => {
  const card_detail = [
    ...
    },
    {
      ...
      description: (
        <>
          Professional results at a fraction of the cost. The most{" "}
          <Link
            href="/blogs/best-resume-maker-for-freshers-in-india-inr100-only-no-subscription"
            className="text-indigo-400 hover:underline"
          >
            affordable resume maker in India
          </Link>{" "}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
          {card_detail.map((item, index) => (
            <div
              key={index}
              className={`group p-4 sm:p-8 rounded-2xl border transition-all duration-300 hover:-translate-y-1 ${item.className} border-white/5 hover:border-white/10 hover:shadow-2xl hover:shadow-indigo-500/10`}
            >
