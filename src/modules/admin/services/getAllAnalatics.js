...
export const getAllAnalytics = async ({ timeRange = "all", customStart, customEnd }) => {
  ...
  try {
    ...
    if (cached) {
      return NextResponse.json(
        new apiResponse(200, "data fetch successfully", JSON.parse(cached)),
        {
          status: 200,
          headers: {
  ...
  const [userStats, resumeStats, paymentStatsRaw, couponStats, todayStats, coverLetterStats] =
    await Promise.all([
      // 1. User Stats
      (async () => {
        ...
        return {
          ...
          activeUsers,
          adminCount,
          userGrowth: userGrowthData.map(i => ({ date: i._id, users: i.users })),
        };
      })(),
      ...
      (async () => {
        const [
          ...
          // Top performing template: most-used template among resumes in range
          Resume.aggregate([
            { $match: timeQuery },
            { $group: { _id: "$ResumeType", count: { $sum: 1 } } },
            { $sort: { count: -1 } },
            { $limit: 1 },
          ]),
        ]);
        return {
          ...
          topTemplateCount: topTemplateData[0]?.count || 0,
        };
      })(),
      ...
      (async () => {
        ...
        const [revResult, avgResult, monthlyData, modesData, paidUsersCount] = await Promise.all([
          ...
          Payment.aggregate([
            { $match: pQuery },
            { $group: { _id: "$paymentMode", count: { $sum: 1 }, revenue: { $sum: "$amount" } } },
            { $sort: { count: -1 } },
          ]),
        ...
        return {
          ...
          monthlyRevenue: monthlyData.map(i => ({
            month: `${i._id.year}-${String(i._id.month).padStart(2, "0")}`,
            revenue: i.revenue,
          })),
          topPaymentModes: modesData.map(i => ({
            mode: i._id || "Other",
            count: i.count,
            revenue: i.revenue,
          })),
      ...
      (async () => {
        ...
        return {
          ...
          couponsByType: byType.map(i => ({ type: i._id, count: i.count })),
          totalDiscount: (totalDisc[0]?.total || 0) / 2,
        };
      })(),
      ...
      // 6. Cover Letter Stats
      (async () => {
        const [totalCoverLetters, paidCoverLetters, draftCoverLetters] = await Promise.all([
          CoverLetter.countDocuments(timeQuery),
          CoverLetter.countDocuments({ ...timeQuery, status: "paid" }),
          CoverLetter.countDocuments({ ...timeQuery, status: "draft" }),
        ]);
        return { totalCoverLetters, paidCoverLetters, draftCoverLetters };
      })(),
    ]);
  ...
  const paymentStats = {
    ...
    arpu:
      userStats.totalUsers > 0
        ? +(paymentStatsRaw.totalRevenue / userStats.totalUsers).toFixed(2)
        : 0,
    conversionRate:
      userStats.totalUsers > 0
        ? +((paymentStatsRaw.paidUsersCount / userStats.totalUsers) * 100).toFixed(2)
        : 0,
  };
