...
import { Send, Sparkles, Lightbulb, RotateCcw, Loader2, Award } from "lucide-react";
...
export default function ResumeChat() {
  ...
  const handleRepeatableSection = (answer) => {
    ...
    const isAskingToRepeat = lastBotMsg?.text?.includes("add another") || lastBotMsg?.text?.includes("Add another");
    
    if (isAskingToRepeat || (isFirstField && currentStep.type === "repeatable" && (answerLower === "yes" || answerLower === "no"))) {
         if (answerLower === "no") {
            moveToNextStep();
            return true;
         }
  ...
  const finishResume = async () => {
     addBotMessage("🎉 Awesome! I'm compiling your resume now using our AI engine... This might take a few seconds.");
    setIsGenerating(true);
    try {
       const res=await axios.post("/api/gen/aiResume",{resumeData:store.answer})
  ...
  useEffect(() => {
    if (store.messages.length === 0) {
      addBotMessage("Hi there! I'm your AI Resume Assistant. I'll help you build a professional resume in minutes. Let's start with your *Full Name*.");
    }
  }, []);
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-slate-50 overflow-hidden">
        ...
        <div className="hidden lg:flex w-80 flex-col border-r border-slate-200 bg-white h-full p-6">
            <div className="bg-gradient-to-br from-indigo-600 to-violet-600 text-white p-6 rounded-2xl shadow-lg mb-8">
                ...
                <p className="text-indigo-100 text-sm leading-relaxed">
                    I'm here to guide you through building a ATS-friendly resume. I'll ask you questions one by one.
                </p>
                <div className="mt-4 pt-4 border-t border-white/10 flex items-center justify-between text-xs font-medium text-indigo-50">
                    <span>Step {store.step + 1} of {flow.length}</span>
                    <span className="bg-white/20 px-2 py-0.5 rounded text-white">{Math.round(((store.step + 1) / flow.length) * 100)}%</span>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                <div className="space-y-3">
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-sm text-slate-600">
                        <p>Keep your answers concise and professional. Use action verbs like "Managed", "Created", "Led".</p>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-sm text-slate-600">
                        <p>If you don't have information for a section, you can type "Skip".</p>
                    </div>
                </div>
            </div>
